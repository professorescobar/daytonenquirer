...
const PERSONA_TO_BEAT = new Map();
const BEAT_TO_SECTION = {
  'general-local': 'local',
  government: 'local',
  crime: 'local',
  education: 'local',
...
for (const [beat, personas] of Object.entries(PERSONA_OPTIONS_BY_BEAT)) {
  for (const persona of personas) {
    if (!PERSONA_TO_BEAT.has(persona.value)) {
      PERSONA_TO_BEAT.set(persona.value, beat);
    }
  }
}
...
function getPersonaBeat(id) {
  return PERSONA_TO_BEAT.get(id) || null;
}

function getPersonaSection(id) {
  const beat = getPersonaBeat(id);
  return beat ? (BEAT_TO_SECTION[beat] || null) : null;
}
