...
async function loadLayer6PersonaConfig(sql: any, personaId: string): Promise<Layer6PersonaConfig> {
  ...
  return {
    ...
    quotaPostgresImageDaily: Math.min(Math.max(Number(row.quotaPostgresImageDaily || 200), 0), 5000),
    quotaSourcedImageDaily: Math.min(Math.max(Number(row.quotaSourcedImageDaily || 120), 0), 5000),
    quotaGeneratedImageDaily: Math.min(Math.max(Number(row.quotaGeneratedImageDaily || 30), 0), 5000),
    quotaTextOnlyDaily: Math.min(Math.max(Number(row.quotaTextOnlyDaily || 400), 0), 5000),
