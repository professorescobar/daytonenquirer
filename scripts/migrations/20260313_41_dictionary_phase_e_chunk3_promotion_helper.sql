-- Phase E / Chunk 3: canonical mutation + provenance promotion helper
-- Safe to run multiple times.

CREATE OR REPLACE FUNCTION dictionary.phase_e_slugify_entity_name(name TEXT, proposal_key TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT
    CASE
      WHEN trim(COALESCE(name, '')) = '' THEN 'entity-' || substr(md5(COALESCE(proposal_key, '')), 1, 8)
      ELSE
        left(
          trim(both '-' FROM regexp_replace(lower(name), '[^a-z0-9]+', '-', 'g')),
          80
        ) || '-' || substr(md5(COALESCE(proposal_key, '')), 1, 8)
    END
$$;

CREATE OR REPLACE FUNCTION dictionary.phase_e_assertion_validity_status(
  effective_start_at TIMESTAMPTZ,
  effective_end_at TIMESTAMPTZ,
  term_end_at TIMESTAMPTZ,
  observed_at TIMESTAMPTZ
)
RETURNS dictionary.assertion_validity_status
LANGUAGE plpgsql
STABLE
AS $$
BEGIN
  IF effective_start_at IS NOT NULL AND effective_start_at > NOW() THEN
    RETURN 'scheduled';
  END IF;

  IF (effective_end_at IS NOT NULL AND effective_end_at < NOW())
     OR (term_end_at IS NOT NULL AND term_end_at < NOW())
  THEN
    RETURN 'expired';
  END IF;

  IF (effective_start_at IS NOT NULL AND effective_start_at <= NOW())
     OR (observed_at IS NOT NULL AND observed_at <= NOW())
  THEN
    RETURN 'current';
  END IF;
...
COMMENT ON FUNCTION dictionary.phase_e_slugify_entity_name(TEXT, TEXT) IS
  'Builds deterministic unique entity slugs for first-pass Phase E entity creation.';

COMMENT ON FUNCTION dictionary.phase_e_assertion_validity_status(TIMESTAMPTZ, TIMESTAMPTZ, TIMESTAMPTZ, TIMESTAMPTZ) IS
  'Derives first-pass assertion validity status for Phase E promotion based on effective/term/observed timestamps.';

COMMENT ON FUNCTION dictionary.phase_e_promote_merge_proposal(UUID, UUID, UUID) IS
  'Applies one Phase E promotable proposal into canonical head, writes provenance in the same statement transaction, and records the promotion ledger result.';
