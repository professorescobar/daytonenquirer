...
COMMENT ON FUNCTION dictionary.phase_e_promote_artifact_run(UUID, UUID, UUID, UUID) IS
  'Promotes the full Phase E artifact/run batch inside one statement transaction so canonical head mutation cannot partially commit for a failing artifact-run, scoped by the Phase D pipeline run id captured on latest validation details.';
...
COMMENT ON FUNCTION dictionary.phase_e_promote_and_publish_artifact_run(UUID, UUID, UUID, UUID) IS
  'Applies the Phase E artifact-run promotion batch and publishes the resulting immutable snapshot in one statement transaction, scoped by the Phase D pipeline run id captured on latest validation details.';
