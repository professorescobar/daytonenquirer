-- Phase E / Chunk 3 follow-up: atomic artifact-run promotion + concurrency locking
...
BEGIN
  FOR proposal_row IN
    ...
    WHERE p.root_source_id = p_root_source_id
      ...
      AND p.validation_substrate_run_id = p_validation_substrate_run_id
...
COMMENT ON FUNCTION dictionary.phase_e_promote_artifact_run(UUID, UUID, UUID, UUID) IS
  'Promotes the full Phase E artifact/run batch inside one statement transaction so canonical head mutation cannot partially commit for a failing artifact-run.';
