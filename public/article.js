...
const PERSONA_AVATARS = {
  ...
  'tsuki-tamara': '/images/personas/gaming-controller.svg',
  // Add other persona values and their corresponding image paths here
  // e.g., 'local-reporter': '/images/personas/reporter.svg',
  default: '/images/personas/default-avatar.svg'
};

function getPersonaAvatar(persona) {
  const key = String(persona || '').trim();
  return PERSONA_AVATARS[key] || PERSONA_AVATARS.default;
}
...
function renderTopicEngine(article) {
  ...
  const avatarUrl = getPersonaAvatar(article.persona);
      ...
      <div class="topic-engine-wrap">
        <div class="topic-engine-header">
          <div class="topic-engine-avatar">
            <img src="${escapeHtml(avatarUrl)}" alt="Author persona icon" />
          </div>
          <div>
            <h3>Ask the Author</h3>
            <p class="topic-engine-disclosure">
              This article was generated by a topic engine. You can ask it questions based on the text above.
            </p>
          </div>
        </div>
